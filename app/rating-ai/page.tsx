...
export default function Page() {
  ...
  const [generatedIdeas, setGeneratedIdeas] = useState<ParsedIdeas | null>(null);
  ...
  useEffect(() => {
    if (completion) {
      try {
        const content = completion.trim().replace("idea:", "");
        const parsedIdeas: ParsedIdeas = JSON.parse(content);
          ...
          {generatedIdeas && (
            <>
              ...
              <CardContent className="grid gap-4">
                <p className="text-left font-medium text-slate-700">Rating</p>
                    ...
                    <div className="w-full dark:bg-gray-500 rounded h-2.5 bg-gray-300 mr-2">
                      <div
                        className={`h-2.5 rounded ${
                          generatedIdeas.rating >= 7
                            ? "bg-green-400" // Green for high ratings (7-10)
                            : generatedIdeas.rating >= 4
                            ? "bg-orange-400" // Yellow for medium ratings (4-6)
                            : "bg-red-400" // Red for low ratings (1-3)
                        }`}
                        style={{ width: `${generatedIdeas.rating * 10}%` }}
                      ></div>
                    </div>
                  </div>
                  <span className="text-sm font-medium text-slate-600">
                    {generatedIdeas.rating}/10
                  </span>
                </div>

                <p className="text-left font-medium text-slate-700">Feedback</p>

                <div>
                  {generatedIdeas.feedback.map((item: string) => (
                    <div key={item} className="items-start ms-4 pb-3 last:mb-0 last:pb-0">
                      <div className="space-y-1">
                        <p className="text-sm text-left font-medium">{item}</p>
                      </div>
                    </div>
                  ))}
                </div>
